...
calculateHorizontalDepth dir = do
   State $ \(Position d h a) -> case dir of
                    Forward x -> let newPos = Position (d + (a * x)) (h + x) a in (depth newPos * horizontal newPos, newPos)
                    Up x -> let newPos = Position d h (a - x) in (depth newPos * horizontal newPos, newPos)
                    Down x -> let newPos = Position d h (a + x) in (depth newPos * horizontal newPos, newPos)

main :: IO Int
main = do
   st <- readFile "input"
   return $ last $ eval (mapM calculateHorizontalDepth (mapToDirection st)) (Position 0 0 0)
